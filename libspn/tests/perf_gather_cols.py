...
class TestResults:
    ...
    def print(self):
        def print_header(dev):
            print1("%3s %11s: %5s %11s %15s %14s" %
                   (dev, 'op', 'size', 'setup_time',
                    'first_run_time', 'rest_run_time'))

        def print_res(res):
            """Helper function printing a single result."""
            print1("%15s: %5d %11.2f %15.2f %14.2f" %
...
class PerformanceTest:
    ...
    def __init__(self, num_param_rows, num_param_cols, num_indices,
                 num_ops, num_runs, dtype, log_devs):
    ...
    def _run_2d(self):
        ...
        r = self._run_test('2d_opt_%sindices' % self.num_param_cols,
                           [Ops.custom, Ops.gather_nd, Ops.indexing],
                           params, indices)
        results.append(r)
