...
class PerformanceTest:
    ...
    def _true_output(self, network_fun, inputs, num_input_vals, num_mixtures,
                     num_subsets, inf_type=None):
        input_size = inputs.shape[0]
        true_out = np.ones((input_size, 1))

        sum_weight = 1.0 / num_input_vals
        product_value = pow(sum_weight, num_subsets)

        if inf_type == spn.InferenceType.MARGINAL:
            return true_out * product_value
        elif inf_type == spn.InferenceType.MPE:
            num_prods = pow(num_mixtures, num_subsets)
            root_weight = 1.0 / num_prods
            return true_out * (product_value * root_weight)
        else:
    ...
    def run(self):
        ...
        inputs = np.random.randint(self.num_input_vals, size=(self.num_input_rows,
                                                              self.num_input_vars))
