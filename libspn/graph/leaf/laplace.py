...
class LaplaceLeaf(LocationScaleLeaf):
    ...
    """A node representing uni-variate Laplace distributions for continuous input
    variables. Each variable will have ``num_components`` components. Each component has its
    ...
    feeding :py:attr:`~libspn.LaplaceLeaf.evidence`. By default, evidence is set to ``True``
    for all variables.
