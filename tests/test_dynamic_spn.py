...
class TestDynamicSPN(tftest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.data_1_steps = get_discrete_data(num_vars=1 * NUM_VARS).reshape(
            (-1, 1, NUM_VARS)
        )
        cls.data_2_steps = get_discrete_data(num_vars=2 * NUM_VARS).reshape(
            (-1, 2, NUM_VARS)
        )

    def test_partition_1_step_adds_up_to_one(self):
        log_values = self.dynamic_spn(
            [self.data_1_steps, [1] * self.data_1_steps.shape[0]]
        )
    ...
    def test_pad_seqs(self):
        ...
        log_values_1_padded = self.dynamic_spn(
            [data_1_padded, [1] * self.data_1_steps.shape[0]]
        )
        log_values_2_padded = self.dynamic_spn(
            [data_2_padded, [2] * self.data_2_steps.shape[0]]
        )
        log_values_concat = self.dynamic_spn(
            [
                data_concat,
                [1] * self.data_1_steps.shape[0] + [2] * self.data_2_steps.shape[0],
            ]
        )
