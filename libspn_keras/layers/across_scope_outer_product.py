...
class AcrossScopeOuterProduct(keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        if self._num_scopes_in % self._num_factors != 0:
            raise ValueError("NUmber of input scopes is not divisible by factor")
        self._num_scopes = self._num_scopes_in // self._num_factors
        self._num_products = self._num_nodes_in ** self._num_factors
        super(AcrossScopeOuterProduct, self).build(input_shape)

    def call(self, x):
        # Split in list of tensors which will be added up using outer products
        shape = [self._num_scopes, self._num_factors, self._num_decomps, -1, self._num_nodes_in]

        log_prob_per_in_scope = tf.split(
            tf.reshape(x, shape=shape), axis=1, num_or_size_splits=self._num_factors)
        ...
        log_prob_per_in_scope = [
            tf.reshape(
                ...
                [1 if j != i else self._num_nodes_in for j in range(self._num_factors)]
            )
